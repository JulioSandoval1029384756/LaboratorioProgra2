package com.example.calcuotas.views
...
fun ContentHomeView(paddingValues: PaddingValues) {
    ...
    var montoInteres by remember { mutableStateOf(value = 0.0) }
    var montoCuota by remember { mutableStateOf(value = 0.0) }

    var showAlert by remember { mutableStateOf(value = false) }
    ...
    Column (modifier = Modifier
        ...
        MainButton(text = "Calcular"){
            if(montoPrestamo != "" && cantCuotas != "") {
                montoInteres =
                    calcularTotal(montoPrestamo.toDouble(), cantCuotas.toInt(), tasa.toDouble())
                montoCuota =
                    calcularCuota(montoPrestamo.toDouble(), cantCuotas.toInt(), tasa.toDouble())
            }else{
                showAlert = true
            }
        }
